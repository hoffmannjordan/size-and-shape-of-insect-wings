...
if __name__=='__main__':
	...
	for filetodo in file_list_seg:
		...
		grayscale_data_scan = import_data('./b3.png',0.25)
		...
		np.savetxt('./b3_masked.csv', new_file.astype(int), fmt='%i', delimiter=',')

	file_list_seg = ["./b4_FMM_seg2.csv"]
	for filetodo in file_list_seg:
		orig_data_scan = np.genfromtxt(filetodo, delimiter=',')
		orig_data_scan = np.asarray(orig_data_scan)
		orig_data_scan = orig_data_scan.astype(int)
		grayscale_data_scan = import_data('./b4.png',0.25)
		tmp = grayscale_data_scan
		tmp2 = np.ones(np.shape(orig_data_scan))
		mask = breadth_search(0, 0)
		print "DID MASK"
		mask = refine_mask(orig_data_scan,mask)
		new_file = np.multiply(mask, orig_data_scan)
		np.savetxt('./b4_masked.csv', new_file.astype(int), fmt='%i', delimiter=',')

	file_list_seg = ["./b5_FMM_seg2.csv"]
	for filetodo in file_list_seg:
		orig_data_scan = np.genfromtxt(filetodo, delimiter=',')
		orig_data_scan = np.asarray(orig_data_scan)
		orig_data_scan = orig_data_scan.astype(int)
		grayscale_data_scan = import_data('./b5.png',0.25)
		tmp = grayscale_data_scan
		tmp2 = np.ones(np.shape(orig_data_scan))
		mask = breadth_search(0, 0)
		print "DID MASK"
		mask = refine_mask(orig_data_scan,mask)
		new_file = np.multiply(mask, orig_data_scan)
		np.savetxt('./b5_masked.csv', new_file.astype(int), fmt='%i', delimiter=',')
